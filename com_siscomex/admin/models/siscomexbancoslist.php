 ...
 * @subpackage 	Siscomex
 */
class SiscomexModelSiscomexbancoslist extends JModelList{
	...
	protected function getListQuery(){
		//$db = JFactory::getDBO();
		$query = 'SELECT `t1`.*  FROM `#__siscomex_bancos` `t1`  ' . $this->_buildQueryWhere() . $this->_buildQueryOrderBy();
		return $query;
	}
	...
	public function getGenericFieldName($fieldName){
		$db = JFactory::getDBO();
		$db->setQuery( 'SELECT codigo AS value `$fieldName` AS text FROM `#__siscomex_bancos` ORDER BY `$fieldName`');
		$options = array();
