 ...
 * @subpackage 	Siscomex
 */
class SiscomexModelSiscomexnaladincca extends JModelAdmin{
	/**
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		return JFactory::getUser()->authorise('core.edit', 'com_siscomex.siscomexnaladincca.'.((int) isset($data[$key]) ? $data[$key] : 0)) or parent::allowEdit($data, $key);
	}
	 ...
	 * @since	1.6
	 */
	public function getTable($type = 'Siscomexnaladincca', $prefix = 'SiscomexTable', $config = array()) 
	{
	...
	{
		// Get the form.
		$form = $this->loadForm('com_siscomex.siscomexnaladincca', 'siscomexnaladincca', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form)) 
		{
			return false;
		}
	...
	{
		return 'administrator/components/com_siscomex/models/forms/siscomexnaladincca.js';
	}
	...
	protected function loadFormData() 
	{
		...
		$data = JFactory::getApplication()->getUserState('com_siscomex.edit.siscomexnaladincca.data', array());
		if (empty($data)) 
		{
			$data = $this->getItem();
		}
