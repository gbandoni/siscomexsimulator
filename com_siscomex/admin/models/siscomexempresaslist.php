 ...
 * @subpackage 	Siscomex
 */
class SiscomexModelSiscomexempresaslist extends JModelList{
	...
	protected function populateState($ordering = null, $direction = null){
		...
		$filter_order = $this->getUserStateFromRequest($this->context . '.filter.ordering', 'filter_order', 'codigo');
	...
	private function _buildQueryOrderBy() {
	    ...
	    $allowedOrders = explode(',', 'codigo,tipopesssoa,razaosocial,telefone,endereco,numero,complemento,bairro,municipio,estado,cep,pais,email'); 
	...
	private function _buildQueryWhere() {
	    ...
	    if ($search){
			...
			$allowedSearch = explode(',', 'tipopesssoa,razaosocial,telefone,endereco,numero,complemento,bairro,municipio,estado,cep,pais,email');
	...
	protected function getListQuery(){
		//$db = JFactory::getDBO();
		$query = 'SELECT `t1`.*  FROM `#__siscomex_empresas` `t1`  ' . $this->_buildQueryWhere() . $this->_buildQueryOrderBy();
		return $query;
	}
	...
	public function getGenericFieldName($fieldName){
		$db = JFactory::getDBO();
		$db->setQuery( 'SELECT codigo AS value `$fieldName` AS text FROM `#__siscomex_empresas` ORDER BY `$fieldName`');
		$options = array();
