 ...
 * @subpackage 	Siscomex
 */
class SiscomexModelSiscomextarifasadi extends JModelAdmin{
	/**
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		return JFactory::getUser()->authorise('core.edit', 'com_siscomex.siscomextarifasadi.'.((int) isset($data[$key]) ? $data[$key] : 0)) or parent::allowEdit($data, $key);
	}
    ...
    protected function prepareTable($table){
        ...
        $nullable ='codigoassuntovinculado,dataanoatovinculado,numeroatovinculado,numeroexatovinculado,siglaorgaoatovinculado,siglatipoatovinculado';
		if(!$nullable) return;
	 ...
	 * @since	1.6
	 */
	public function getTable($type = 'Siscomextarifasadi', $prefix = 'SiscomexTable', $config = array()) 
	{
	...
	{
		// Get the form.
		$form = $this->loadForm('com_siscomex.siscomextarifasadi', 'siscomextarifasadi', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form)) 
		{
			return false;
		}
	...
	{
		return 'administrator/components/com_siscomex/models/forms/siscomextarifasadi.js';
	}
	...
	protected function loadFormData() 
	{
		...
		$data = JFactory::getApplication()->getUserState('com_siscomex.edit.siscomextarifasadi.data', array());
		if (empty($data)) 
		{
			$data = $this->getItem();
		}
