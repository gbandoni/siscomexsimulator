 ...
 * @subpackage 	Siscomex
 */
class SiscomexModelSiscomexadicao extends JModelAdmin{
	/**
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		return JFactory::getUser()->authorise('core.edit', 'com_siscomex.siscomexadicao.'.((int) isset($data[$key]) ? $data[$key] : 0)) or parent::allowEdit($data, $key);
	}
    ...
    protected function prepareTable($table){
        ...
        $nullable ='codigoacordoaladi,codigoaplicacaomercadoria,codigoausenciafabricante,codigocoberturacambial,codigomoedanegociada,codigotipoacordotarifario,codigovinculoimportadorexportador,indicadorbemencomenda,indicadormaterialusado,indicadormultimodal,indicadortipocertificado,nomelocalcondicaovenda,numerodocumentoreducao,numeroidentificacaoli,numerorof,percentualcoeficientereducaoii,pesoliquidomercadoria,quantidadeunidadeestatistica,textocomplementovaloraduaneiro,valoraliquotaicms,valorcalculodcrdolar,valorfinanciadosuperior360,valorfretemercadoriamoedanacional,valorfretemercadoriamoedanegociada,valoriicalculadodcrmoedanacional,valoriidevidozfm,valoriiareceberzfm,valormercadoriacondicaovenda,valormercadoriaembarquemoedanacional,valormercadoriavendamoedanacional,valorseguromercadoriamoedanacional,valorseguromercadoriamoedanegociada';
		if(!$nullable) return;
	 ...
	 * @since	1.6
	 */
	public function getTable($type = 'Siscomexadicao', $prefix = 'SiscomexTable', $config = array()) 
	{
	...
	{
		// Get the form.
		$form = $this->loadForm('com_siscomex.siscomexadicao', 'siscomexadicao', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form)) 
		{
			return false;
		}
	...
	{
		return 'administrator/components/com_siscomex/models/forms/siscomexadicao.js';
	}
	...
	protected function loadFormData() 
	{
		...
		$data = JFactory::getApplication()->getUserState('com_siscomex.edit.siscomexadicao.data', array());
		if (empty($data)) 
		{
			$data = $this->getItem();
		}
