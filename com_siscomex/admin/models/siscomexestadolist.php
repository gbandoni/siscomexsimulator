 ...
 * @subpackage 	Siscomex
 */
class SiscomexModelSiscomexestadolist extends JModelList{
	...
	protected function populateState($ordering = null, $direction = null){
		...
		$filter_order = $this->getUserStateFromRequest($this->context . '.filter.ordering', 'filter_order', 'estado_id');
	...
	private function _buildQueryOrderBy() {
	    ...
	    $allowedOrders = explode(',', 'estado_id,sigla,nome'); 
	...
	private function _buildQueryWhere() {
	    ...
	    if ($search){
			...
			$allowedSearch = explode(',', 'estado_id,sigla,nome');
	...
	protected function getListQuery(){
		//$db = JFactory::getDBO();
		$query = 'SELECT `t1`.*  FROM `#__siscomex_estado` `t1`  ' . $this->_buildQueryWhere() . $this->_buildQueryOrderBy();
		return $query;
	}
	...
	public function getGenericFieldName($fieldName){
		$db = JFactory::getDBO();
		$db->setQuery( 'SELECT estado_id AS value `$fieldName` AS text FROM `#__siscomex_estado` ORDER BY `$fieldName`');
		$options = array();
