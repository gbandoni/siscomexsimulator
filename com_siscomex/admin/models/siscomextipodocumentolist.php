 ...
 * @subpackage 	Siscomex
 */
class SiscomexModelSiscomextipodocumentolist extends JModelList{
	...
	private function _buildQueryOrderBy() {
	    ...
	    $allowedOrders = explode(',', 'codigo,sigla,descricao,maritima,fluvia,aerea,rodoviaria,ferroviaria,postal,lacustre'); 
	...
	private function _buildQueryWhere() {
	    ...
	    if ($search){
			...
			$allowedSearch = explode(',', 'codigo,sigla,descricao,maritima,fluvia,aerea,rodoviaria,ferroviaria,postal,lacustre');
	...
	protected function getListQuery(){
		//$db = JFactory::getDBO();
		$query = 'SELECT `t1`.*  FROM `#__siscomex_tipo_documento` `t1`  ' . $this->_buildQueryWhere() . $this->_buildQueryOrderBy();
		return $query;
	}
	...
	public function getGenericFieldName($fieldName){
		$db = JFactory::getDBO();
		$db->setQuery( 'SELECT codigo AS value `$fieldName` AS text FROM `#__siscomex_tipo_documento` ORDER BY `$fieldName`');
		$options = array();
