 ...
 * @subpackage 	Siscomex
 */
class SiscomexModelSiscomexdi extends JModelAdmin{
	/**
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		return JFactory::getUser()->authorise('core.edit', 'com_siscomex.siscomexdi.'.((int) isset($data[$key]) ? $data[$key] : 0)) or parent::allowEdit($data, $key);
	}
    ...
    protected function prepareTable($table){
        ...
        $nullable ='codigotipodocumentocarga,codigomotivotransmissao,cargapesobruto,cargapesoliquido,codigobandeiratranspote,codigosetorarmazenamento,codigotipoagentecarga,codigotipoconsignatario,codigotipopagamentotributario,codigoutilizacaodocumentocarga,datachegadacarga,dataembarque,identificacaodeclaracaoimportacao,indicadormultimodalviatransporte,indicadoroperacaofundap,informacoescomplementares,nomelocalembarque,numeroagentecarga,numeroconsignatario,numerocontapagamentotributario,numerocpfrepresentantelegal,numerodocumentocargamaster,numeroimportador,numeromanifesto,totaladicoes,valorfreteterritorionacionalmoedanegociada,valortotalfretecollect,valortotalfretemoedanacional,valortotalfreteprepaid,valortotalmlemoedanacional,valortotalseguromoedanacional,valortotalseguromoedanegociada';
		if(!$nullable) return;
	 ...
	 * @since	1.6
	 */
	public function getTable($type = 'Siscomexdi', $prefix = 'SiscomexTable', $config = array()) 
	{
	...
	{
		// Get the form.
		$form = $this->loadForm('com_siscomex.siscomexdi', 'siscomexdi', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form)) 
		{
			return false;
		}
	...
	{
		return 'administrator/components/com_siscomex/models/forms/siscomexdi.js';
	}
	...
	protected function loadFormData() 
	{
		...
		$data = JFactory::getApplication()->getUserState('com_siscomex.edit.siscomexdi.data', array());
		if (empty($data)) 
		{
			$data = $this->getItem();
		}
