 ...
 * @subpackage 	Siscomex
 */
class SiscomexModelSiscomexespecncmlist extends JModelList{
	...
	protected function populateState($ordering = null, $direction = null){
		...
		$filter_order = $this->getUserStateFromRequest($this->context . '.filter.ordering', 'filter_order', 'ncm');
	...
	private function _buildQueryOrderBy() {
	    ...
	    $allowedOrders = explode(',', 'ncm,atributo,especificacao,nivel,descricao'); 
	...
	private function _buildQueryWhere() {
	    ...
	    if ($search){
			...
			$allowedSearch = explode(',', 'ncm,atributo,especificacao,nivel,descricao');
	...
	protected function getListQuery(){
		//$db = JFactory::getDBO();
		$query = 'SELECT `t1`.*  FROM `#__siscomex_espec_ncm` `t1`  ' . $this->_buildQueryWhere() . $this->_buildQueryOrderBy();
		return $query;
	}
	...
	public function getGenericFieldName($fieldName){
		$db = JFactory::getDBO();
		$db->setQuery( 'SELECT ncm AS value `$fieldName` AS text FROM `#__siscomex_espec_ncm` ORDER BY `$fieldName`');
		$options = array();
