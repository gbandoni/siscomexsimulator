 ...
 * @subpackage 	Siscomex
 */
class SiscomexModelSiscomexcidadeslist extends JModelList{
	...
	protected function populateState($ordering = null, $direction = null){
		...
		$filter_order = $this->getUserStateFromRequest($this->context . '.filter.ordering', 'filter_order', 'codmun');
	...
	private function _buildQueryOrderBy() {
	    ...
	    $allowedOrders = explode(',', 'estado,codmun,nome'); 
	...
	private function _buildQueryWhere() {
	    ...
	    if ($search){
			...
			$allowedSearch = explode(',', 'estado,codmun,nome');
	...
	protected function getListQuery(){
		//$db = JFactory::getDBO();
		$query = 'SELECT `t1`.*  FROM `#__siscomex_cidades` `t1`  ' . $this->_buildQueryWhere() . $this->_buildQueryOrderBy();
		return $query;
	}
	...
	public function getGenericFieldName($fieldName){
		$db = JFactory::getDBO();
		$db->setQuery( 'SELECT codmun AS value `$fieldName` AS text FROM `#__siscomex_cidades` ORDER BY `$fieldName`');
		$options = array();
