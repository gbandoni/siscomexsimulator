 ...
 * @subpackage 	Siscomex
 */
class SiscomexModelSiscomexcasoslist extends JModelList{
	...
	private function _buildQueryOrderBy() {
	    ...
	    $allowedOrders = explode(',', 'id,idcriador,datacriacao,teste,published,global,descricao,exclusivo'); 
	...
	protected function getListQuery(){
		//$db = JFactory::getDBO();
		$query = 'SELECT `t1`.*  FROM `#__siscomex_casos` `t1`  ' . $this->_buildQueryWhere() . $this->_buildQueryOrderBy();
		return $query;
	}
	...
	public function getGenericFieldName($fieldName){
		$db = JFactory::getDBO();
		$db->setQuery( 'SELECT id AS value `$fieldName` AS text FROM `#__siscomex_casos` ORDER BY `$fieldName`');
		$options = array();
