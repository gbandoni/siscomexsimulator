 ...
 * @subpackage 	Siscomex
 */
class SiscomexModelSiscomexfundlegallist extends JModelList{
	...
	private function _buildQueryOrderBy() {
	    ...
	    $allowedOrders = explode(',', 'codigo,descricao,reg_tribut_1,reg_tribut_2,reg_tribut_3,reg_tribut_4'); 
	...
	private function _buildQueryWhere() {
	    ...
	    if ($search){
			...
			$allowedSearch = explode(',', 'codigo,descricao');
	...
	protected function getListQuery(){
		//$db = JFactory::getDBO();
		$query = 'SELECT `t1`.*  FROM `#__siscomex_fund_legal` `t1`  ' . $this->_buildQueryWhere() . $this->_buildQueryOrderBy();
		return $query;
	}
	...
	public function getGenericFieldName($fieldName){
		$db = JFactory::getDBO();
		$db->setQuery( 'SELECT codigo AS value `$fieldName` AS text FROM `#__siscomex_fund_legal` ORDER BY `$fieldName`');
		$options = array();
