 ...
 * @subpackage 	Siscomex
 */
class SiscomexModelSiscomexincoterms extends JModelAdmin{
	/**
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		return JFactory::getUser()->authorise('core.edit', 'com_siscomex.siscomexincoterms.'.((int) isset($data[$key]) ? $data[$key] : 0)) or parent::allowEdit($data, $key);
	}
	 ...
	 * @since	1.6
	 */
	public function getTable($type = 'Siscomexincoterms', $prefix = 'SiscomexTable', $config = array()) 
	{
	...
	{
		// Get the form.
		$form = $this->loadForm('com_siscomex.siscomexincoterms', 'siscomexincoterms', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form)) 
		{
			return false;
		}
	...
	{
		return 'administrator/components/com_siscomex/models/forms/siscomexincoterms.js';
	}
	...
	protected function loadFormData() 
	{
		...
		$data = JFactory::getApplication()->getUserState('com_siscomex.edit.siscomexincoterms.data', array());
		if (empty($data)) 
		{
			$data = $this->getItem();
		}
