 ...
 * @subpackage 	Siscomex
 */
class SiscomexModelSiscomexrecintoalfandegadolist extends JModelList{
	...
	private function _buildQueryOrderBy() {
	    ...
	    $allowedOrders = explode(',', 'codigo,regiao,nomeporto,estado,terminal,descricao'); 
	...
	private function _buildQueryWhere() {
	    ...
	    if ($search){
			...
			$allowedSearch = explode(',', 'codigo,regiao,nomeporto,estado,terminal,descricao');
	...
	protected function getListQuery(){
		//$db = JFactory::getDBO();
		$query = 'SELECT `t1`.*  FROM `#__siscomex_recintoalfandegado` `t1`  ' . $this->_buildQueryWhere() . $this->_buildQueryOrderBy();
		return $query;
	}
	...
	public function getGenericFieldName($fieldName){
		$db = JFactory::getDBO();
		$db->setQuery( 'SELECT codigo AS value `$fieldName` AS text FROM `#__siscomex_recintoalfandegado` ORDER BY `$fieldName`');
		$options = array();
