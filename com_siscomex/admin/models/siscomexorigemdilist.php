 ...
 * @subpackage 	Siscomex
 */
class SiscomexModelSiscomexorigemdilist extends JModelList{
	...
	private function _buildQueryWhere() {
	    ...
	    if ($search){
			...
			$allowedSearch = explode(',', 'ID,descricao');
	...
	protected function getListQuery(){
		//$db = JFactory::getDBO();
		$query = 'SELECT `t1`.*  FROM `#__siscomex_origem_di` `t1`  ' . $this->_buildQueryWhere() . $this->_buildQueryOrderBy();
		return $query;
	}
	...
	public function getGenericFieldName($fieldName){
		$db = JFactory::getDBO();
		$db->setQuery( 'SELECT ID AS value `$fieldName` AS text FROM `#__siscomex_origem_di` ORDER BY `$fieldName`');
		$options = array();
