 ...
 * @subpackage 	Siscomex
 */
class SiscomexModelSiscomexatribncmlist extends JModelList{
	...
	private function _buildQueryOrderBy() {
	    ...
	    $allowedOrders = explode(',', 'ncm,atributoncm,multiplo,nivel,atributo'); 
	...
	private function _buildQueryWhere() {
	    ...
	    if ($search){
			...
			$allowedSearch = explode(',', 'ncm,atributoncm,multiplo,nivel,atributo');
	...
	protected function getListQuery(){
		//$db = JFactory::getDBO();
		$query = 'SELECT `t1`.*  FROM `#__siscomex_atrib_ncm` `t1`  ' . $this->_buildQueryWhere() . $this->_buildQueryOrderBy();
		return $query;
	}
	...
	public function getGenericFieldName($fieldName){
		$db = JFactory::getDBO();
		$db->setQuery( 'SELECT ncm AS value `$fieldName` AS text FROM `#__siscomex_atrib_ncm` ORDER BY `$fieldName`');
		$options = array();
