 ...
 * @subpackage 	Siscomex
 */
class SiscomexModelSiscomexmoedaslist extends JModelList{
	...
	protected function populateState($ordering = null, $direction = null){
		...
		$filter_order = $this->getUserStateFromRequest($this->context . '.filter.ordering', 'filter_order', 'idmoeda');
	...
	private function _buildQueryOrderBy() {
	    ...
	    $allowedOrders = explode(',', 'idmoeda,codigo,nome,simbolo,pais,tipo,dataexclusao'); 
	...
	private function _buildQueryWhere() {
	    ...
	    if ($search){
			...
			$allowedSearch = explode(',', 'nome,simbolo,pais,tipo');
	...
	protected function getListQuery(){
		//$db = JFactory::getDBO();
		$query = 'SELECT `t1`.*  FROM `#__siscomex_moedas` `t1`  ' . $this->_buildQueryWhere() . $this->_buildQueryOrderBy();
		return $query;
	}
	...
	public function getGenericFieldName($fieldName){
		$db = JFactory::getDBO();
		$db->setQuery( 'SELECT idmoeda AS value `$fieldName` AS text FROM `#__siscomex_moedas` ORDER BY `$fieldName`');
		$options = array();
