 ...
 * @subpackage 	Siscomex
 */
class SiscomexModelSiscomextipodeclaracaolist extends JModelList{
	...
	protected function populateState($ordering = null, $direction = null){
		...
		$filter_order = $this->getUserStateFromRequest($this->context . '.filter.ordering', 'filter_order', 'codigo_tipo');
	...
	private function _buildQueryOrderBy() {
	    ...
	    $allowedOrders = explode(',', 'codigo_tipo,tipo_declaracao,descricao_tipo'); 
	...
	private function _buildQueryWhere() {
	    ...
	    if ($search){
			...
			$allowedSearch = explode(',', 'tipo_declaracao,descricao_tipo');
	...
	protected function getListQuery(){
		//$db = JFactory::getDBO();
		$query = 'SELECT `t1`.*  FROM `#__siscomex_tipo_declaracao` `t1`  ' . $this->_buildQueryWhere() . $this->_buildQueryOrderBy();
		return $query;
	}
	...
	public function getGenericFieldName($fieldName){
		$db = JFactory::getDBO();
		$db->setQuery( 'SELECT codigo_tipo AS value `$fieldName` AS text FROM `#__siscomex_tipo_declaracao` ORDER BY `$fieldName`');
		$options = array();
