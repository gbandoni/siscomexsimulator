 ...
 * @subpackage 	Siscomex
 */
class SiscomexModelSiscomexlocalidadeembarquelist extends JModelList{
	...
	private function _buildQueryOrderBy() {
	    ...
	    $allowedOrders = explode(',', 'ID,codpais,localidade'); 
	...
	private function _buildQueryWhere() {
	    ...
	    if ($search){
			...
			$allowedSearch = explode(',', 'codpais,localidade');
	...
	protected function getListQuery(){
		//$db = JFactory::getDBO();
		$query = 'SELECT `t1`.*  FROM `#__siscomex_localidadeembarque` `t1`  ' . $this->_buildQueryWhere() . $this->_buildQueryOrderBy();
		return $query;
	}
	...
	public function getGenericFieldName($fieldName){
		$db = JFactory::getDBO();
		$db->setQuery( 'SELECT ID AS value `$fieldName` AS text FROM `#__siscomex_localidadeembarque` ORDER BY `$fieldName`');
		$options = array();
