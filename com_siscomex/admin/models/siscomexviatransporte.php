 ...
 * @subpackage 	Siscomex
 */
class SiscomexModelSiscomexviatransporte extends JModelAdmin{
	/**
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		return JFactory::getUser()->authorise('core.edit', 'com_siscomex.siscomexviatransporte.'.((int) isset($data[$key]) ? $data[$key] : 0)) or parent::allowEdit($data, $key);
	}
	 ...
	 * @since	1.6
	 */
	public function getTable($type = 'Siscomexviatransporte', $prefix = 'SiscomexTable', $config = array()) 
	{
	...
	{
		// Get the form.
		$form = $this->loadForm('com_siscomex.siscomexviatransporte', 'siscomexviatransporte', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form)) 
		{
			return false;
		}
	...
	{
		return 'administrator/components/com_siscomex/models/forms/siscomexviatransporte.js';
	}
	...
	protected function loadFormData() 
	{
		...
		$data = JFactory::getApplication()->getUserState('com_siscomex.edit.siscomexviatransporte.data', array());
		if (empty($data)) 
		{
			$data = $this->getItem();
		}
