 ...
 * @subpackage 	Siscomex
 */
class SiscomexModelSiscomextarifasadilist extends JModelList{
	...
	protected function populateState($ordering = null, $direction = null){
		...
		$filter_order = $this->getUserStateFromRequest($this->context . '.filter.ordering', 'filter_order', 'numerodocumentocarga');
	...
	private function _buildQueryOrderBy() {
	    ...
	    $allowedOrders = explode(',', 'numerodocumentocarga,numeroadicao,codigoassuntovinculado,dataanoatovinculado,numeroatovinculado,numeroexatovinculado,siglaorgaoatovinculado,siglatipoatovinculado'); 
	...
	private function _buildQueryWhere() {
	    ...
	    if ($search){
			...
			$allowedSearch = explode(',', 'numerodocumentocarga,numeroadicao,codigoassuntovinculado,dataanoatovinculado,numeroatovinculado,numeroexatovinculado,siglaorgaoatovinculado,siglatipoatovinculado');
	...
	protected function getListQuery(){
		//$db = JFactory::getDBO();
		$query = 'SELECT `t1`.*  FROM `#__siscomex_tarifas_adi` `t1`  ' . $this->_buildQueryWhere() . $this->_buildQueryOrderBy();
		return $query;
	}
	...
	public function getGenericFieldName($fieldName){
		$db = JFactory::getDBO();
		$db->setQuery( 'SELECT numerodocumentocarga AS value `$fieldName` AS text FROM `#__siscomex_tarifas_adi` ORDER BY `$fieldName`');
		$options = array();
