 ...
 * @subpackage 	Siscomex
 */
class SiscomexViewSiscomexmotivotransmissaolist extends JViewLegacy
{
	/**
	 * Siscomexmotivotransmissaolist view display method
	 * @return void
	 **/
	function display($tpl = null){
		...
		JToolBarHelper::title( JText::_( 'COM_SISCOMEX_MENU_SISCOMEXMOTIVOTRANSMISSAOLIST' ), 'generic.png' );
		if($this->user->authorise('core.edit', 'com_siscomex')) JToolBarHelper::editList('siscomexmotivotransmissao.edit');
		if($this->user->authorise('core.create', 'com_siscomex')) JToolBarHelper::addNew('siscomexmotivotransmissao.add');
		if($this->user->authorise('core.delete', 'com_siscomex')) JToolBarHelper::deleteList('', 'siscomexmotivotransmissaolist.delete');
		
		if( (isset($this->rows[0]->published)) && ($this->user->authorise('core.edit', 'com_siscomex')) ){
			JToolBarHelper::divider();
			JToolBarHelper::publish('siscomexmotivotransmissaolist.publish');
			JToolBarHelper::unpublish('siscomexmotivotransmissaolist.unpublish');
		}
	...
	protected function auth($area){
		//echo '<pre>' . print_r(JAccess::getActions('com_siscomex','siscomexmotivotransmissao'),true).'</pre>';die();
		$aclLocal = array();
		foreach(JAccess::getActions('com_siscomex', 'siscomexmotivotransmissao') as $ar) $aclLocal[] = $ar->name;
		if(in_array($area, $aclLocal)){
			return $this->user->authorise($area, 'com_siscomex', 'siscomexmotivotransmissao');
		}else{
			$aclGlobal = array();
