...
<?php endif; ?>
	...
	<th class="jcb_fieldDiv jcb_fieldLabel"><?php $numCols++; ?>
		<?php echo JHTML::_('grid.sort', JText::_('COM_SISCOMEX_SISCOMEXMOTIVOTRANSMISSAO_CODIGO_LABEL'), 'codigo', $this->lists['order_Dir'], $this->lists['order']); ?>
	</th>
	<th class="jcb_fieldDiv jcb_fieldLabel"><?php $numCols++; ?>
		<?php echo JHTML::_('grid.sort', JText::_('COM_SISCOMEX_SISCOMEXMOTIVOTRANSMISSAO_DESCRICAO_LABEL'), 'descricao', $this->lists['order_Dir'], $this->lists['order']); ?>
	</th>
			...
			<th width="20" class="jcb_fieldDiv jcb_fieldLabel nowrap"><?php $numCols++; ?>
				...
				<?php if ($this->auth('core.edit') && ($this->lists['order']=='ordering')): ?>
					<?php echo JHtml::_('grid.order',  $this->rows, 'filesave.png', 'siscomexmotivotransmissaolist.saveorder'); ?>
				<?php endif;?>				
			</th>
			<?php endif; ?>
	...
	for ($i=0, $n=count( $this->rows ); $i < $n; $i++)	{
		...
		$link = JRoute::_( 'index.php?option=com_siscomex&task=siscomexmotivotransmissao.edit&codigo='. $row->codigo);
		$k = 1 - $k;
		?>
		<tr class="<?php echo "row$k"; ?>">
			...
			<td>
				<?php if (isset($row->checked_out) && ($row->checked_out)) : ?>
					<?php echo JHtml::_('jgrid.checkedout', $i, 'someone...', $row->checked_out_time, 'siscomexmotivotransmissaolist.', $this->auth('siscomexmotivotransmissao.checkin')); ?>
				<?php endif; ?>				
	...
	<td class="jcb_fieldDiv jcb_fieldValue">
		<?php echo $row->descricao; ?>
			...
			<td class="order nowrap">
				...
				?>
					<span><?php echo $page->orderUpIcon( $i, $i>0, 'siscomexmotivotransmissaolist.'.$up, JText::_( 'JLIB_HTML_MOVE_UP' ), $enable ); ?></span>
					<span><?php echo $page->orderDownIcon( $i, $n, $i<$n, 'siscomexmotivotransmissaolist.'.$down, JText::_( 'JLIB_HTML_MOVE_DOWN' ), $enable ); ?></span>					
			...
			<td>
				<?php if($this->j3x): ?>
					<?php echo JHtml::_('jgrid.published', $row->published, $i, 'siscomexmotivotransmissaolist.', true); ?>
				<?php else: ?>
					<?php echo JHTML::_('grid.published', $row, $i, 'tick.png', 'publish_x.png', 'siscomexmotivotransmissaolist.'); ?>
				<?php endif; ?>
			</td>
			<?php endif; ?>
		</tr>
...
<input type="hidden" name="view" value="siscomexmotivotransmissaolist" />
