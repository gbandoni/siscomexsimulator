 ...
 * @subpackage 	Siscomex
 */
class SiscomexViewSiscomexdoccargadi extends JViewLegacy
{
	/**
	...
	function display($tpl = null){
		...
		JToolBarHelper::title(   JText::_( 'COM_SISCOMEX_MENU_SISCOMEXDOCCARGADILIST' ).': <span>[ ' . $text.' ]</span>' );
		
		if ($isNew)  {
			if($user->authorise('core.create', 'com_siscomex')){
				JToolBarHelper::apply('siscomexdoccargadi.apply');
				JToolBarHelper::save('siscomexdoccargadi.save');
			}
			JToolBarHelper::cancel('siscomexdoccargadi.cancel');
		} else {
			...
			JToolBarHelper::cancel( 'siscomexdoccargadi.cancel', 'JTOOLBAR_CLOSE' );
		}
