 ...
 * @subpackage 	Siscomex
 */
class SiscomexViewSiscomextipodocumento extends JViewLegacy
{
	/**
	...
	function display($tpl = null){
		...
		JToolBarHelper::title(   JText::_( 'COM_SISCOMEX_MENU_SISCOMEXTIPODOCUMENTOLIST' ).': <span>[ ' . $text.' ]</span>' );
		
		if ($isNew)  {
			if($user->authorise('core.create', 'com_siscomex')){
				JToolBarHelper::apply('siscomextipodocumento.apply');
				JToolBarHelper::save('siscomextipodocumento.save');
			}
			JToolBarHelper::cancel('siscomextipodocumento.cancel');
		} else {
			...
			JToolBarHelper::cancel( 'siscomextipodocumento.cancel', 'JTOOLBAR_CLOSE' );
		}
