 ...
 * @subpackage 	Siscomex
 */
class SiscomexViewSiscomexempresas extends JViewLegacy
{
	/**
	...
	function display($tpl = null){
		...
		JToolBarHelper::title(   JText::_( 'COM_SISCOMEX_MENU_SISCOMEXEMPRESASLIST' ).': <span>[ ' . $text.' ]</span>' );
		
		if ($isNew)  {
			if($user->authorise('core.create', 'com_siscomex')){
				JToolBarHelper::apply('siscomexempresas.apply');
				JToolBarHelper::save('siscomexempresas.save');
			}
			JToolBarHelper::cancel('siscomexempresas.cancel');
		} else {
			...
			JToolBarHelper::cancel( 'siscomexempresas.cancel', 'JTOOLBAR_CLOSE' );
		}
