 ...
 * @subpackage 	Siscomex
 */
class SiscomexViewSiscomexlocalidadeembarquelist extends JViewLegacy
{
	/**
	 * Siscomexlocalidadeembarquelist view display method
	 * @return void
	 **/
	function display($tpl = null){
		...
		JToolBarHelper::title( JText::_( 'COM_SISCOMEX_MENU_SISCOMEXLOCALIDADEEMBARQUELIST' ), 'generic.png' );
		if($this->user->authorise('core.edit', 'com_siscomex')) JToolBarHelper::editList('siscomexlocalidadeembarque.edit');
		if($this->user->authorise('core.create', 'com_siscomex')) JToolBarHelper::addNew('siscomexlocalidadeembarque.add');
		if($this->user->authorise('core.delete', 'com_siscomex')) JToolBarHelper::deleteList('', 'siscomexlocalidadeembarquelist.delete');
		
		if( (isset($this->rows[0]->published)) && ($this->user->authorise('core.edit', 'com_siscomex')) ){
			JToolBarHelper::divider();
			JToolBarHelper::publish('siscomexlocalidadeembarquelist.publish');
			JToolBarHelper::unpublish('siscomexlocalidadeembarquelist.unpublish');
		}
	...
	protected function auth($area){
		//echo '<pre>' . print_r(JAccess::getActions('com_siscomex','siscomexlocalidadeembarque'),true).'</pre>';die();
		$aclLocal = array();
		foreach(JAccess::getActions('com_siscomex', 'siscomexlocalidadeembarque') as $ar) $aclLocal[] = $ar->name;
		if(in_array($area, $aclLocal)){
			return $this->user->authorise($area, 'com_siscomex', 'siscomexlocalidadeembarque');
		}else{
			$aclGlobal = array();
