 ...
 * @subpackage 	Siscomex
 */
class SiscomexViewSiscomexcidades extends JViewLegacy
{
	/**
	...
	function display($tpl = null){
		...
		$isNew = ($this->item->codmun == null);

		$text = $isNew ? JText::_( 'JACTION_CREATE' ) : JText::_( 'JACTION_EDIT' );
		JToolBarHelper::title(   JText::_( 'COM_SISCOMEX_MENU_SISCOMEXCIDADESLIST' ).': <span>[ ' . $text.' ]</span>' );
		
		if ($isNew)  {
			if($user->authorise('core.create', 'com_siscomex')){
				JToolBarHelper::apply('siscomexcidades.apply');
				JToolBarHelper::save('siscomexcidades.save');
			}
			JToolBarHelper::cancel('siscomexcidades.cancel');
		} else {
			...
			JToolBarHelper::cancel( 'siscomexcidades.cancel', 'JTOOLBAR_CLOSE' );
		}
