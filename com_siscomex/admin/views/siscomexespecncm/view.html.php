 ...
 * @subpackage 	Siscomex
 */
class SiscomexViewSiscomexespecncm extends JViewLegacy
{
	/**
	...
	function display($tpl = null){
		...
		JToolBarHelper::title(   JText::_( 'COM_SISCOMEX_MENU_SISCOMEXESPECNCMLIST' ).': <span>[ ' . $text.' ]</span>' );
		
		if ($isNew)  {
			if($user->authorise('core.create', 'com_siscomex')){
				JToolBarHelper::apply('siscomexespecncm.apply');
				JToolBarHelper::save('siscomexespecncm.save');
			}
			JToolBarHelper::cancel('siscomexespecncm.cancel');
		} else {
			...
			JToolBarHelper::cancel( 'siscomexespecncm.cancel', 'JTOOLBAR_CLOSE' );
		}
