 ...
 * @subpackage 	Siscomex
 */
class SiscomexViewSiscomexnaladincca extends JViewLegacy
{
	/**
	...
	function display($tpl = null){
		...
		JToolBarHelper::title(   JText::_( 'COM_SISCOMEX_MENU_SISCOMEXNALADINCCALIST' ).': <span>[ ' . $text.' ]</span>' );
		
		if ($isNew)  {
			if($user->authorise('core.create', 'com_siscomex')){
				JToolBarHelper::apply('siscomexnaladincca.apply');
				JToolBarHelper::save('siscomexnaladincca.save');
			}
			JToolBarHelper::cancel('siscomexnaladincca.cancel');
		} else {
			...
			JToolBarHelper::cancel( 'siscomexnaladincca.cancel', 'JTOOLBAR_CLOSE' );
		}
