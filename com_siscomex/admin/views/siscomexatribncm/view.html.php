 ...
 * @subpackage 	Siscomex
 */
class SiscomexViewSiscomexatribncm extends JViewLegacy
{
	/**
	...
	function display($tpl = null){
		...
		$isNew = ($this->item->ncm == null);

		$text = $isNew ? JText::_( 'JACTION_CREATE' ) : JText::_( 'JACTION_EDIT' );
		JToolBarHelper::title(   JText::_( 'COM_SISCOMEX_MENU_SISCOMEXATRIBNCMLIST' ).': <span>[ ' . $text.' ]</span>' );
		
		if ($isNew)  {
			if($user->authorise('core.create', 'com_siscomex')){
				JToolBarHelper::apply('siscomexatribncm.apply');
				JToolBarHelper::save('siscomexatribncm.save');
			}
			JToolBarHelper::cancel('siscomexatribncm.cancel');
		} else {
			...
			JToolBarHelper::cancel( 'siscomexatribncm.cancel', 'JTOOLBAR_CLOSE' );
		}
