 ...
 * @subpackage 	Siscomex
 */
class SiscomexViewSiscomexacordoaladi extends JViewLegacy
{
	/**
	...
	function display($tpl = null){
		...
		JToolBarHelper::title(   JText::_( 'COM_SISCOMEX_MENU_SISCOMEXACORDOALADILIST' ).': <span>[ ' . $text.' ]</span>' );
		
		if ($isNew)  {
			if($user->authorise('core.create', 'com_siscomex')){
				JToolBarHelper::apply('siscomexacordoaladi.apply');
				JToolBarHelper::save('siscomexacordoaladi.save');
			}
			JToolBarHelper::cancel('siscomexacordoaladi.cancel');
		} else {
			...
			JToolBarHelper::cancel( 'siscomexacordoaladi.cancel', 'JTOOLBAR_CLOSE' );
		}
