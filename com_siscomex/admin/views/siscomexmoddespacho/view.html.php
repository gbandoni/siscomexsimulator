 ...
 * @subpackage 	Siscomex
 */
class SiscomexViewSiscomexmoddespacho extends JViewLegacy
{
	/**
	...
	function display($tpl = null){
		...
		$isNew = ($this->item->codigo_modalidade == null);

		$text = $isNew ? JText::_( 'JACTION_CREATE' ) : JText::_( 'JACTION_EDIT' );
		JToolBarHelper::title(   JText::_( 'COM_SISCOMEX_MENU_SISCOMEXMODDESPACHOLIST' ).': <span>[ ' . $text.' ]</span>' );
		
		if ($isNew)  {
			if($user->authorise('core.create', 'com_siscomex')){
				JToolBarHelper::apply('siscomexmoddespacho.apply');
				JToolBarHelper::save('siscomexmoddespacho.save');
			}
			JToolBarHelper::cancel('siscomexmoddespacho.cancel');
		} else {
			...
			JToolBarHelper::cancel( 'siscomexmoddespacho.cancel', 'JTOOLBAR_CLOSE' );
		}
