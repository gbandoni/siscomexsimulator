 ...
 * @subpackage 	Siscomex
 */
class SiscomexViewSiscomexvaloraduaneira extends JViewLegacy
{
	/**
	...
	function display($tpl = null){
		...
		JToolBarHelper::title(   JText::_( 'COM_SISCOMEX_MENU_SISCOMEXVALORADUANEIRALIST' ).': <span>[ ' . $text.' ]</span>' );
		
		if ($isNew)  {
			if($user->authorise('core.create', 'com_siscomex')){
				JToolBarHelper::apply('siscomexvaloraduaneira.apply');
				JToolBarHelper::save('siscomexvaloraduaneira.save');
			}
			JToolBarHelper::cancel('siscomexvaloraduaneira.cancel');
		} else {
			...
			JToolBarHelper::cancel( 'siscomexvaloraduaneira.cancel', 'JTOOLBAR_CLOSE' );
		}
