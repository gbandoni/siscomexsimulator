 ...
 * @subpackage 	Siscomex
 */
class SiscomexViewSiscomexmotadmtemp extends JViewLegacy
{
	/**
	...
	function display($tpl = null){
		...
		JToolBarHelper::title(   JText::_( 'COM_SISCOMEX_MENU_SISCOMEXMOTADMTEMPLIST' ).': <span>[ ' . $text.' ]</span>' );
		
		if ($isNew)  {
			if($user->authorise('core.create', 'com_siscomex')){
				JToolBarHelper::apply('siscomexmotadmtemp.apply');
				JToolBarHelper::save('siscomexmotadmtemp.save');
			}
			JToolBarHelper::cancel('siscomexmotadmtemp.cancel');
		} else {
			...
			JToolBarHelper::cancel( 'siscomexmotadmtemp.cancel', 'JTOOLBAR_CLOSE' );
		}
