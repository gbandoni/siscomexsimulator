 ...
 * @subpackage 	Siscomex
 */
class SiscomexViewSiscomexdeducoes extends JViewLegacy
{
	/**
	...
	function display($tpl = null){
		...
		JToolBarHelper::title(   JText::_( 'COM_SISCOMEX_MENU_SISCOMEXDEDUCOESLIST' ).': <span>[ ' . $text.' ]</span>' );
		
		if ($isNew)  {
			if($user->authorise('core.create', 'com_siscomex')){
				JToolBarHelper::apply('siscomexdeducoes.apply');
				JToolBarHelper::save('siscomexdeducoes.save');
			}
			JToolBarHelper::cancel('siscomexdeducoes.cancel');
		} else {
			...
			JToolBarHelper::cancel( 'siscomexdeducoes.cancel', 'JTOOLBAR_CLOSE' );
		}
