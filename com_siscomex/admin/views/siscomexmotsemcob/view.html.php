 ...
 * @subpackage 	Siscomex
 */
class SiscomexViewSiscomexmotsemcob extends JViewLegacy
{
	/**
	...
	function display($tpl = null){
		...
		JToolBarHelper::title(   JText::_( 'COM_SISCOMEX_MENU_SISCOMEXMOTSEMCOBLIST' ).': <span>[ ' . $text.' ]</span>' );
		
		if ($isNew)  {
			if($user->authorise('core.create', 'com_siscomex')){
				JToolBarHelper::apply('siscomexmotsemcob.apply');
				JToolBarHelper::save('siscomexmotsemcob.save');
			}
			JToolBarHelper::cancel('siscomexmotsemcob.cancel');
		} else {
			...
			JToolBarHelper::cancel( 'siscomexmotsemcob.cancel', 'JTOOLBAR_CLOSE' );
		}
