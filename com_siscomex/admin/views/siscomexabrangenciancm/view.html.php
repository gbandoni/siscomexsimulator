 ...
 * @subpackage 	Siscomex
 */
class SiscomexViewSiscomexabrangenciancm extends JViewLegacy
{
	/**
	...
	function display($tpl = null){
		...
		JToolBarHelper::title(   JText::_( 'COM_SISCOMEX_MENU_SISCOMEXABRANGENCIANCMLIST' ).': <span>[ ' . $text.' ]</span>' );
		
		if ($isNew)  {
			if($user->authorise('core.create', 'com_siscomex')){
				JToolBarHelper::apply('siscomexabrangenciancm.apply');
				JToolBarHelper::save('siscomexabrangenciancm.save');
			}
			JToolBarHelper::cancel('siscomexabrangenciancm.cancel');
		} else {
			...
			JToolBarHelper::cancel( 'siscomexabrangenciancm.cancel', 'JTOOLBAR_CLOSE' );
		}
