 ...
 * @subpackage 	Siscomex
 */
class SiscomexViewSiscomexorigemdi extends JViewLegacy
{
	/**
	...
	function display($tpl = null){
		...
		JToolBarHelper::title(   JText::_( 'COM_SISCOMEX_MENU_SISCOMEXORIGEMDILIST' ).': <span>[ ' . $text.' ]</span>' );
		
		if ($isNew)  {
			if($user->authorise('core.create', 'com_siscomex')){
				JToolBarHelper::apply('siscomexorigemdi.apply');
				JToolBarHelper::save('siscomexorigemdi.save');
			}
			JToolBarHelper::cancel('siscomexorigemdi.cancel');
		} else {
			...
			JToolBarHelper::cancel( 'siscomexorigemdi.cancel', 'JTOOLBAR_CLOSE' );
		}
