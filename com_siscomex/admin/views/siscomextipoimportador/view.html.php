 ...
 * @subpackage 	Siscomex
 */
class SiscomexViewSiscomextipoimportador extends JViewLegacy
{
	/**
	...
	function display($tpl = null){
		...
		JToolBarHelper::title(   JText::_( 'COM_SISCOMEX_MENU_SISCOMEXTIPOIMPORTADORLIST' ).': <span>[ ' . $text.' ]</span>' );
		
		if ($isNew)  {
			if($user->authorise('core.create', 'com_siscomex')){
				JToolBarHelper::apply('siscomextipoimportador.apply');
				JToolBarHelper::save('siscomextipoimportador.save');
			}
			JToolBarHelper::cancel('siscomextipoimportador.cancel');
		} else {
			...
			JToolBarHelper::cancel( 'siscomextipoimportador.cancel', 'JTOOLBAR_CLOSE' );
		}
