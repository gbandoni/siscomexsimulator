 ...
 * @subpackage 	Siscomex
 */
class SiscomexViewSiscomextipodocumentodes extends JViewLegacy
{
	/**
	...
	function display($tpl = null){
		...
		JToolBarHelper::title(   JText::_( 'COM_SISCOMEX_MENU_SISCOMEXTIPODOCUMENTODESLIST' ).': <span>[ ' . $text.' ]</span>' );
		
		if ($isNew)  {
			if($user->authorise('core.create', 'com_siscomex')){
				JToolBarHelper::apply('siscomextipodocumentodes.apply');
				JToolBarHelper::save('siscomextipodocumentodes.save');
			}
			JToolBarHelper::cancel('siscomextipodocumentodes.cancel');
		} else {
			...
			JToolBarHelper::cancel( 'siscomextipodocumentodes.cancel', 'JTOOLBAR_CLOSE' );
		}
