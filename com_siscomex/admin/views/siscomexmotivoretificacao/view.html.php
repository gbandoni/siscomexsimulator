 ...
 * @subpackage 	Siscomex
 */
class SiscomexViewSiscomexmotivoretificacao extends JViewLegacy
{
	/**
	...
	function display($tpl = null){
		...
		$isNew = ($this->item->codigo_motivo == null);

		$text = $isNew ? JText::_( 'JACTION_CREATE' ) : JText::_( 'JACTION_EDIT' );
		JToolBarHelper::title(   JText::_( 'COM_SISCOMEX_MENU_SISCOMEXMOTIVORETIFICACAOLIST' ).': <span>[ ' . $text.' ]</span>' );
		
		if ($isNew)  {
			if($user->authorise('core.create', 'com_siscomex')){
				JToolBarHelper::apply('siscomexmotivoretificacao.apply');
				JToolBarHelper::save('siscomexmotivoretificacao.save');
			}
			JToolBarHelper::cancel('siscomexmotivoretificacao.cancel');
		} else {
			...
			JToolBarHelper::cancel( 'siscomexmotivoretificacao.cancel', 'JTOOLBAR_CLOSE' );
		}
