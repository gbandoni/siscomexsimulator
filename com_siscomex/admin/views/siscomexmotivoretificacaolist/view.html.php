 ...
 * @subpackage 	Siscomex
 */
class SiscomexViewSiscomexmotivoretificacaolist extends JViewLegacy
{
	/**
	 * Siscomexmotivoretificacaolist view display method
	 * @return void
	 **/
	function display($tpl = null){
		...
		JToolBarHelper::title( JText::_( 'COM_SISCOMEX_MENU_SISCOMEXMOTIVORETIFICACAOLIST' ), 'generic.png' );
		if($this->user->authorise('core.edit', 'com_siscomex')) JToolBarHelper::editList('siscomexmotivoretificacao.edit');
		if($this->user->authorise('core.create', 'com_siscomex')) JToolBarHelper::addNew('siscomexmotivoretificacao.add');
		if($this->user->authorise('core.delete', 'com_siscomex')) JToolBarHelper::deleteList('', 'siscomexmotivoretificacaolist.delete');
		
		if( (isset($this->rows[0]->published)) && ($this->user->authorise('core.edit', 'com_siscomex')) ){
			JToolBarHelper::divider();
			JToolBarHelper::publish('siscomexmotivoretificacaolist.publish');
			JToolBarHelper::unpublish('siscomexmotivoretificacaolist.unpublish');
		}
		...
		//$default_order_field = 'codigo_motivo';
	...
	protected function auth($area){
		//echo '<pre>' . print_r(JAccess::getActions('com_siscomex','siscomexmotivoretificacao'),true).'</pre>';die();
		$aclLocal = array();
		foreach(JAccess::getActions('com_siscomex', 'siscomexmotivoretificacao') as $ar) $aclLocal[] = $ar->name;
		if(in_array($area, $aclLocal)){
			return $this->user->authorise($area, 'com_siscomex', 'siscomexmotivoretificacao');
		}else{
			$aclGlobal = array();
