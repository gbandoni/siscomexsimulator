 ...
 * @subpackage 	Siscomex
 */
class SiscomexViewSiscomexviatransporte extends JViewLegacy
{
	/**
	...
	function display($tpl = null){
		...
		JToolBarHelper::title(   JText::_( 'COM_SISCOMEX_MENU_SISCOMEXVIATRANSPORTELIST' ).': <span>[ ' . $text.' ]</span>' );
		
		if ($isNew)  {
			if($user->authorise('core.create', 'com_siscomex')){
				JToolBarHelper::apply('siscomexviatransporte.apply');
				JToolBarHelper::save('siscomexviatransporte.save');
			}
			JToolBarHelper::cancel('siscomexviatransporte.cancel');
		} else {
			...
			JToolBarHelper::cancel( 'siscomexviatransporte.cancel', 'JTOOLBAR_CLOSE' );
		}
