 ...
 * @subpackage 	Siscomex
 */
class SiscomexViewSiscomexmercadoriaadi extends JViewLegacy
{
	/**
	...
	function display($tpl = null){
		...
		$isNew = ($this->item->numerodocumentocarga == null);

		$text = $isNew ? JText::_( 'JACTION_CREATE' ) : JText::_( 'JACTION_EDIT' );
		JToolBarHelper::title(   JText::_( 'COM_SISCOMEX_MENU_SISCOMEXMERCADORIAADILIST' ).': <span>[ ' . $text.' ]</span>' );
		
		if ($isNew)  {
			if($user->authorise('core.create', 'com_siscomex')){
				JToolBarHelper::apply('siscomexmercadoriaadi.apply');
				JToolBarHelper::save('siscomexmercadoriaadi.save');
			}
			JToolBarHelper::cancel('siscomexmercadoriaadi.cancel');
		} else {
			...
			JToolBarHelper::cancel( 'siscomexmercadoriaadi.cancel', 'JTOOLBAR_CLOSE' );
		}
