 ...
 * @subpackage 	Siscomex
 */
class SiscomexViewSiscomexregimetribut extends JViewLegacy
{
	/**
	...
	function display($tpl = null){
		...
		JToolBarHelper::title(   JText::_( 'COM_SISCOMEX_MENU_SISCOMEXREGIMETRIBUTLIST' ).': <span>[ ' . $text.' ]</span>' );
		
		if ($isNew)  {
			if($user->authorise('core.create', 'com_siscomex')){
				JToolBarHelper::apply('siscomexregimetribut.apply');
				JToolBarHelper::save('siscomexregimetribut.save');
			}
			JToolBarHelper::cancel('siscomexregimetribut.cancel');
		} else {
			...
			JToolBarHelper::cancel( 'siscomexregimetribut.cancel', 'JTOOLBAR_CLOSE' );
		}
