...
class SiscomexViewSiscomexpaises extends JViewLegacy
{
	...
	function display($tpl = null){
		...
		$isNew = ($this->item->codigo_pais == null);

		$text = $isNew ? JText::_( 'JACTION_CREATE' ) : JText::_( 'JACTION_EDIT' );
		JToolBarHelper::title(   JText::_( 'COM_SISCOMEX_MENU_SISCOMEXPAISESLIST' ).': <span>[ ' . $text.' ]</span>' );
		
		if ($isNew)  {
			if($user->authorise('core.create', 'com_siscomex')){
				JToolBarHelper::apply('siscomexpaises.apply');
				JToolBarHelper::save('siscomexpaises.save');
			}
			JToolBarHelper::cancel('siscomexpaises.cancel');
		} else {
			...
			JToolBarHelper::cancel( 'siscomexpaises.cancel', 'JTOOLBAR_CLOSE' );
		}
