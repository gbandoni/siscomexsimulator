 ...
 * @subpackage 	Siscomex
 */
class SiscomexViewSiscomextiporecipiente extends JViewLegacy
{
	/**
	...
	function display($tpl = null){
		...
		JToolBarHelper::title(   JText::_( 'COM_SISCOMEX_MENU_SISCOMEXTIPORECIPIENTELIST' ).': <span>[ ' . $text.' ]</span>' );
		
		if ($isNew)  {
			if($user->authorise('core.create', 'com_siscomex')){
				JToolBarHelper::apply('siscomextiporecipiente.apply');
				JToolBarHelper::save('siscomextiporecipiente.save');
			}
			JToolBarHelper::cancel('siscomextiporecipiente.cancel');
		} else {
			...
			JToolBarHelper::cancel( 'siscomextiporecipiente.cancel', 'JTOOLBAR_CLOSE' );
		}
