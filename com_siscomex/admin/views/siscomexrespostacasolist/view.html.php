 ...
 * @subpackage 	Siscomex
 */
class SiscomexViewSiscomexrespostacasolist extends JViewLegacy
{
	/**
	 * Siscomexrespostacasolist view display method
	 * @return void
	 **/
	function display($tpl = null){
		...
		JToolBarHelper::title( JText::_( 'COM_SISCOMEX_MENU_SISCOMEXRESPOSTACASOLIST' ), 'generic.png' );
		if($this->user->authorise('core.edit', 'com_siscomex')) JToolBarHelper::editList('siscomexrespostacaso.edit');
		if($this->user->authorise('core.create', 'com_siscomex')) JToolBarHelper::addNew('siscomexrespostacaso.add');
		if($this->user->authorise('core.delete', 'com_siscomex')) JToolBarHelper::deleteList('', 'siscomexrespostacasolist.delete');
		
		if( (isset($this->rows[0]->published)) && ($this->user->authorise('core.edit', 'com_siscomex')) ){
			JToolBarHelper::divider();
			JToolBarHelper::publish('siscomexrespostacasolist.publish');
			JToolBarHelper::unpublish('siscomexrespostacasolist.unpublish');
		}
	...
	protected function auth($area){
		//echo '<pre>' . print_r(JAccess::getActions('com_siscomex','siscomexrespostacaso'),true).'</pre>';die();
		$aclLocal = array();
		foreach(JAccess::getActions('com_siscomex', 'siscomexrespostacaso') as $ar) $aclLocal[] = $ar->name;
		if(in_array($area, $aclLocal)){
			return $this->user->authorise($area, 'com_siscomex', 'siscomexrespostacaso');
		}else{
			$aclGlobal = array();
