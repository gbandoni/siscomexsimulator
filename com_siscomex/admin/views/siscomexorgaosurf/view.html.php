 ...
 * @subpackage 	Siscomex
 */
class SiscomexViewSiscomexorgaosurf extends JViewLegacy
{
	/**
	...
	function display($tpl = null){
		...
		JToolBarHelper::title(   JText::_( 'COM_SISCOMEX_MENU_SISCOMEXORGAOSURFLIST' ).': <span>[ ' . $text.' ]</span>' );
		
		if ($isNew)  {
			if($user->authorise('core.create', 'com_siscomex')){
				JToolBarHelper::apply('siscomexorgaosurf.apply');
				JToolBarHelper::save('siscomexorgaosurf.save');
			}
			JToolBarHelper::cancel('siscomexorgaosurf.cancel');
		} else {
			...
			JToolBarHelper::cancel( 'siscomexorgaosurf.cancel', 'JTOOLBAR_CLOSE' );
		}
