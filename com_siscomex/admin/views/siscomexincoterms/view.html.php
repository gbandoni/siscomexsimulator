 ...
 * @subpackage 	Siscomex
 */
class SiscomexViewSiscomexincoterms extends JViewLegacy
{
	/**
	...
	function display($tpl = null){
		...
		JToolBarHelper::title(   JText::_( 'COM_SISCOMEX_MENU_SISCOMEXINCOTERMSLIST' ).': <span>[ ' . $text.' ]</span>' );
		
		if ($isNew)  {
			if($user->authorise('core.create', 'com_siscomex')){
				JToolBarHelper::apply('siscomexincoterms.apply');
				JToolBarHelper::save('siscomexincoterms.save');
			}
			JToolBarHelper::cancel('siscomexincoterms.cancel');
		} else {
			...
			JToolBarHelper::cancel( 'siscomexincoterms.cancel', 'JTOOLBAR_CLOSE' );
		}
