 ...
 * @subpackage 	Siscomex
 */
class SiscomexViewSiscomextipoembalagem extends JViewLegacy
{
	/**
	...
	function display($tpl = null){
		...
		JToolBarHelper::title(   JText::_( 'COM_SISCOMEX_MENU_SISCOMEXTIPOEMBALAGEMLIST' ).': <span>[ ' . $text.' ]</span>' );
		
		if ($isNew)  {
			if($user->authorise('core.create', 'com_siscomex')){
				JToolBarHelper::apply('siscomextipoembalagem.apply');
				JToolBarHelper::save('siscomextipoembalagem.save');
			}
			JToolBarHelper::cancel('siscomextipoembalagem.cancel');
		} else {
			...
			JToolBarHelper::cancel( 'siscomextipoembalagem.cancel', 'JTOOLBAR_CLOSE' );
		}
