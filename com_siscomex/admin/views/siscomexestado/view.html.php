 ...
 * @subpackage 	Siscomex
 */
class SiscomexViewSiscomexestado extends JViewLegacy
{
	/**
	...
	function display($tpl = null){
		...
		$isNew = ($this->item->estado_id == null);

		$text = $isNew ? JText::_( 'JACTION_CREATE' ) : JText::_( 'JACTION_EDIT' );
		JToolBarHelper::title(   JText::_( 'COM_SISCOMEX_MENU_SISCOMEXESTADOLIST' ).': <span>[ ' . $text.' ]</span>' );
		
		if ($isNew)  {
			if($user->authorise('core.create', 'com_siscomex')){
				JToolBarHelper::apply('siscomexestado.apply');
				JToolBarHelper::save('siscomexestado.save');
			}
			JToolBarHelper::cancel('siscomexestado.cancel');
		} else {
			...
			JToolBarHelper::cancel( 'siscomexestado.cancel', 'JTOOLBAR_CLOSE' );
		}
