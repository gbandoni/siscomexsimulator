 ...
 * @subpackage 	Siscomex
 */
class SiscomexViewSiscomexcasos extends JViewLegacy
{
	/**
	...
	function display($tpl = null){
		...
		JToolBarHelper::title(   JText::_( 'COM_SISCOMEX_MENU_SISCOMEXCASOSLIST' ).': <span>[ ' . $text.' ]</span>' );
		
		if ($isNew)  {
			if($user->authorise('core.create', 'com_siscomex')){
				JToolBarHelper::apply('siscomexcasos.apply');
				JToolBarHelper::save('siscomexcasos.save');
			}
			JToolBarHelper::cancel('siscomexcasos.cancel');
		} else {
			...
			JToolBarHelper::cancel( 'siscomexcasos.cancel', 'JTOOLBAR_CLOSE' );
		}
