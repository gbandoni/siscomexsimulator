 ...
 * @subpackage 	Siscomex
 */
class SiscomexViewSiscomextarifasadi extends JViewLegacy
{
	/**
	...
	function display($tpl = null){
		...
		JToolBarHelper::title(   JText::_( 'COM_SISCOMEX_MENU_SISCOMEXTARIFASADILIST' ).': <span>[ ' . $text.' ]</span>' );
		
		if ($isNew)  {
			if($user->authorise('core.create', 'com_siscomex')){
				JToolBarHelper::apply('siscomextarifasadi.apply');
				JToolBarHelper::save('siscomextarifasadi.save');
			}
			JToolBarHelper::cancel('siscomextarifasadi.cancel');
		} else {
			...
			JToolBarHelper::cancel( 'siscomextarifasadi.cancel', 'JTOOLBAR_CLOSE' );
		}
