 ...
 * @subpackage 	Siscomex
 */
class SiscomexViewSiscomexaladi extends JViewLegacy
{
	/**
	...
	function display($tpl = null){
		...
		JToolBarHelper::title(   JText::_( 'COM_SISCOMEX_MENU_SISCOMEXALADILIST' ).': <span>[ ' . $text.' ]</span>' );
		
		if ($isNew)  {
			if($user->authorise('core.create', 'com_siscomex')){
				JToolBarHelper::apply('siscomexaladi.apply');
				JToolBarHelper::save('siscomexaladi.save');
			}
			JToolBarHelper::cancel('siscomexaladi.cancel');
		} else {
			...
			JToolBarHelper::cancel( 'siscomexaladi.cancel', 'JTOOLBAR_CLOSE' );
		}
