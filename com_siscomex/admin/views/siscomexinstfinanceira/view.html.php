 ...
 * @subpackage 	Siscomex
 */
class SiscomexViewSiscomexinstfinanceira extends JViewLegacy
{
	/**
	...
	function display($tpl = null){
		...
		JToolBarHelper::title(   JText::_( 'COM_SISCOMEX_MENU_SISCOMEXINSTFINANCEIRALIST' ).': <span>[ ' . $text.' ]</span>' );
		
		if ($isNew)  {
			if($user->authorise('core.create', 'com_siscomex')){
				JToolBarHelper::apply('siscomexinstfinanceira.apply');
				JToolBarHelper::save('siscomexinstfinanceira.save');
			}
			JToolBarHelper::cancel('siscomexinstfinanceira.cancel');
		} else {
			...
			JToolBarHelper::cancel( 'siscomexinstfinanceira.cancel', 'JTOOLBAR_CLOSE' );
		}
