 ...
 * @subpackage 	Siscomex
 */
class SiscomexViewSiscomexmotivotransmissao extends JViewLegacy
{
	/**
	...
	function display($tpl = null){
		...
		JToolBarHelper::title(   JText::_( 'COM_SISCOMEX_MENU_SISCOMEXMOTIVOTRANSMISSAOLIST' ).': <span>[ ' . $text.' ]</span>' );
		
		if ($isNew)  {
			if($user->authorise('core.create', 'com_siscomex')){
				JToolBarHelper::apply('siscomexmotivotransmissao.apply');
				JToolBarHelper::save('siscomexmotivotransmissao.save');
			}
			JToolBarHelper::cancel('siscomexmotivotransmissao.cancel');
		} else {
			...
			JToolBarHelper::cancel( 'siscomexmotivotransmissao.cancel', 'JTOOLBAR_CLOSE' );
		}
