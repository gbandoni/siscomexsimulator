 ...
 * @subpackage 	Siscomex
 */
class SiscomexViewSiscomexadicao extends JViewLegacy
{
	/**
	...
	function display($tpl = null){
		...
		JToolBarHelper::title(   JText::_( 'COM_SISCOMEX_MENU_SISCOMEXADICAOLIST' ).': <span>[ ' . $text.' ]</span>' );
		
		if ($isNew)  {
			if($user->authorise('core.create', 'com_siscomex')){
				JToolBarHelper::apply('siscomexadicao.apply');
				JToolBarHelper::save('siscomexadicao.save');
			}
			JToolBarHelper::cancel('siscomexadicao.cancel');
		} else {
			...
			JToolBarHelper::cancel( 'siscomexadicao.cancel', 'JTOOLBAR_CLOSE' );
		}
