 ...
 * @subpackage 	Siscomex
 */
class SiscomexViewSiscomextipomanifesto extends JViewLegacy
{
	/**
	...
	function display($tpl = null){
		...
		JToolBarHelper::title(   JText::_( 'COM_SISCOMEX_MENU_SISCOMEXTIPOMANIFESTOLIST' ).': <span>[ ' . $text.' ]</span>' );
		
		if ($isNew)  {
			if($user->authorise('core.create', 'com_siscomex')){
				JToolBarHelper::apply('siscomextipomanifesto.apply');
				JToolBarHelper::save('siscomextipomanifesto.save');
			}
			JToolBarHelper::cancel('siscomextipomanifesto.cancel');
		} else {
			...
			JToolBarHelper::cancel( 'siscomextipomanifesto.cancel', 'JTOOLBAR_CLOSE' );
		}
