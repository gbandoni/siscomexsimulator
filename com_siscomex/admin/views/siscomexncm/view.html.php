 ...
 * @subpackage 	Siscomex
 */
class SiscomexViewSiscomexncm extends JViewLegacy
{
	/**
	...
	function display($tpl = null){
		...
		$isNew = ($this->item->codigoncm == null);

		$text = $isNew ? JText::_( 'JACTION_CREATE' ) : JText::_( 'JACTION_EDIT' );
		JToolBarHelper::title(   JText::_( 'COM_SISCOMEX_MENU_SISCOMEXNCMLIST' ).': <span>[ ' . $text.' ]</span>' );
		
		if ($isNew)  {
			if($user->authorise('core.create', 'com_siscomex')){
				JToolBarHelper::apply('siscomexncm.apply');
				JToolBarHelper::save('siscomexncm.save');
			}
			JToolBarHelper::cancel('siscomexncm.cancel');
		} else {
			...
			JToolBarHelper::cancel( 'siscomexncm.cancel', 'JTOOLBAR_CLOSE' );
		}
