 ...
 * @subpackage 	Siscomex
 */
class SiscomexViewSiscomexbancos extends JViewLegacy
{
	/**
	...
	function display($tpl = null){
		...
		JToolBarHelper::title(   JText::_( 'COM_SISCOMEX_MENU_SISCOMEXBANCOSLIST' ).': <span>[ ' . $text.' ]</span>' );
		
		if ($isNew)  {
			if($user->authorise('core.create', 'com_siscomex')){
				JToolBarHelper::apply('siscomexbancos.apply');
				JToolBarHelper::save('siscomexbancos.save');
			}
			JToolBarHelper::cancel('siscomexbancos.cancel');
		} else {
			...
			JToolBarHelper::cancel( 'siscomexbancos.cancel', 'JTOOLBAR_CLOSE' );
		}
