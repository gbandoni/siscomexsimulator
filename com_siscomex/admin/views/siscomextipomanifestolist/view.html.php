...
{
	/**
	 * Siscomextipomanifestolist view display method
	 * @return void
	 **/
	function display($tpl = null){
		...
		JToolBarHelper::title( JText::_( 'COM_SISCOMEX_MENU_SISCOMEXTIPOMANIFESTOLIST' ), 'generic.png' );
		if($this->user->authorise('core.edit', 'com_siscomex')) JToolBarHelper::editList('siscomextipomanifesto.edit');
		if($this->user->authorise('core.create', 'com_siscomex')) JToolBarHelper::addNew('siscomextipomanifesto.add');
		if($this->user->authorise('core.delete', 'com_siscomex')) JToolBarHelper::deleteList('', 'siscomextipomanifestolist.delete');
		
		if( (isset($this->rows[0]->published)) && ($this->user->authorise('core.edit', 'com_siscomex')) ){
			JToolBarHelper::divider();
			JToolBarHelper::publish('siscomextipomanifestolist.publish');
			JToolBarHelper::unpublish('siscomextipomanifestolist.unpublish');
		}
	...
	protected function auth($area){
		//echo '<pre>' . print_r(JAccess::getActions('com_siscomex','siscomextipomanifesto'),true).'</pre>';die();
		$aclLocal = array();
		foreach(JAccess::getActions('com_siscomex', 'siscomextipomanifesto') as $ar) $aclLocal[] = $ar->name;
		if(in_array($area, $aclLocal)){
			return $this->user->authorise($area, 'com_siscomex', 'siscomextipomanifesto');
		}else{
			$aclGlobal = array();
