 ...
 * @subpackage 	Siscomex
 */
class SiscomexViewSiscomexrespostacaso extends JViewLegacy
{
	/**
	...
	function display($tpl = null){
		...
		JToolBarHelper::title(   JText::_( 'COM_SISCOMEX_MENU_SISCOMEXRESPOSTACASOLIST' ).': <span>[ ' . $text.' ]</span>' );
		
		if ($isNew)  {
			if($user->authorise('core.create', 'com_siscomex')){
				JToolBarHelper::apply('siscomexrespostacaso.apply');
				JToolBarHelper::save('siscomexrespostacaso.save');
			}
			JToolBarHelper::cancel('siscomexrespostacaso.cancel');
		} else {
			...
			JToolBarHelper::cancel( 'siscomexrespostacaso.cancel', 'JTOOLBAR_CLOSE' );
		}
