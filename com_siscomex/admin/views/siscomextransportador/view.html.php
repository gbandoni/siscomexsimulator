 ...
 * @subpackage 	Siscomex
 */
class SiscomexViewSiscomextransportador extends JViewLegacy
{
	/**
	...
	function display($tpl = null){
		...
		JToolBarHelper::title(   JText::_( 'COM_SISCOMEX_MENU_SISCOMEXTRANSPORTADORLIST' ).': <span>[ ' . $text.' ]</span>' );
		
		if ($isNew)  {
			if($user->authorise('core.create', 'com_siscomex')){
				JToolBarHelper::apply('siscomextransportador.apply');
				JToolBarHelper::save('siscomextransportador.save');
			}
			JToolBarHelper::cancel('siscomextransportador.cancel');
		} else {
			...
			JToolBarHelper::cancel( 'siscomextransportador.cancel', 'JTOOLBAR_CLOSE' );
		}
