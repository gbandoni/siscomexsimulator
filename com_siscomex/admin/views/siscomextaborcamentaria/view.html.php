 ...
 * @subpackage 	Siscomex
 */
class SiscomexViewSiscomextaborcamentaria extends JViewLegacy
{
	/**
	...
	function display($tpl = null){
		...
		JToolBarHelper::title(   JText::_( 'COM_SISCOMEX_MENU_SISCOMEXTABORCAMENTARIALIST' ).': <span>[ ' . $text.' ]</span>' );
		
		if ($isNew)  {
			if($user->authorise('core.create', 'com_siscomex')){
				JToolBarHelper::apply('siscomextaborcamentaria.apply');
				JToolBarHelper::save('siscomextaborcamentaria.save');
			}
			JToolBarHelper::cancel('siscomextaborcamentaria.cancel');
		} else {
			...
			JToolBarHelper::cancel( 'siscomextaborcamentaria.cancel', 'JTOOLBAR_CLOSE' );
		}
