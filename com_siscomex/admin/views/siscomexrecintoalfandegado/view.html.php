 ...
 * @subpackage 	Siscomex
 */
class SiscomexViewSiscomexrecintoalfandegado extends JViewLegacy
{
	/**
	...
	function display($tpl = null){
		...
		$isNew = ($this->item->codigo == null);

		$text = $isNew ? JText::_( 'JACTION_CREATE' ) : JText::_( 'JACTION_EDIT' );
		JToolBarHelper::title(   JText::_( 'COM_SISCOMEX_MENU_SISCOMEXRECINTOALFANDEGADOLIST' ).': <span>[ ' . $text.' ]</span>' );
		
		if ($isNew)  {
			if($user->authorise('core.create', 'com_siscomex')){
				JToolBarHelper::apply('siscomexrecintoalfandegado.apply');
				JToolBarHelper::save('siscomexrecintoalfandegado.save');
			}
			JToolBarHelper::cancel('siscomexrecintoalfandegado.cancel');
		} else {
			...
			JToolBarHelper::cancel( 'siscomexrecintoalfandegado.cancel', 'JTOOLBAR_CLOSE' );
		}
