 ...
 * @subpackage 	Siscomex
 */
class SiscomexViewSiscomexnaladish extends JViewLegacy
{
	/**
	...
	function display($tpl = null){
		...
		JToolBarHelper::title(   JText::_( 'COM_SISCOMEX_MENU_SISCOMEXNALADISHLIST' ).': <span>[ ' . $text.' ]</span>' );
		
		if ($isNew)  {
			if($user->authorise('core.create', 'com_siscomex')){
				JToolBarHelper::apply('siscomexnaladish.apply');
				JToolBarHelper::save('siscomexnaladish.save');
			}
			JToolBarHelper::cancel('siscomexnaladish.cancel');
		} else {
			...
			JToolBarHelper::cancel( 'siscomexnaladish.cancel', 'JTOOLBAR_CLOSE' );
		}
