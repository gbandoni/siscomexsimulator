 ...
 * @subpackage 	Siscomex
 */
class SiscomexViewSiscomexmoedas extends JViewLegacy
{
	/**
	...
	function display($tpl = null){
		...
		$isNew = ($this->item->idmoeda == null);

		$text = $isNew ? JText::_( 'JACTION_CREATE' ) : JText::_( 'JACTION_EDIT' );
		JToolBarHelper::title(   JText::_( 'COM_SISCOMEX_MENU_SISCOMEXMOEDASLIST' ).': <span>[ ' . $text.' ]</span>' );
		
		if ($isNew)  {
			if($user->authorise('core.create', 'com_siscomex')){
				JToolBarHelper::apply('siscomexmoedas.apply');
				JToolBarHelper::save('siscomexmoedas.save');
			}
			JToolBarHelper::cancel('siscomexmoedas.cancel');
		} else {
			...
			JToolBarHelper::cancel( 'siscomexmoedas.cancel', 'JTOOLBAR_CLOSE' );
		}
