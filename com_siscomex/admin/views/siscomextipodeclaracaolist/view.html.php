 ...
 * @subpackage 	Siscomex
 */
class SiscomexViewSiscomextipodeclaracaolist extends JViewLegacy
{
	/**
	 * Siscomextipodeclaracaolist view display method
	 * @return void
	 **/
	function display($tpl = null){
		...
		JToolBarHelper::title( JText::_( 'COM_SISCOMEX_MENU_SISCOMEXTIPODECLARACAOLIST' ), 'generic.png' );
		if($this->user->authorise('core.edit', 'com_siscomex')) JToolBarHelper::editList('siscomextipodeclaracao.edit');
		if($this->user->authorise('core.create', 'com_siscomex')) JToolBarHelper::addNew('siscomextipodeclaracao.add');
		if($this->user->authorise('core.delete', 'com_siscomex')) JToolBarHelper::deleteList('', 'siscomextipodeclaracaolist.delete');
		
		if( (isset($this->rows[0]->published)) && ($this->user->authorise('core.edit', 'com_siscomex')) ){
			JToolBarHelper::divider();
			JToolBarHelper::publish('siscomextipodeclaracaolist.publish');
			JToolBarHelper::unpublish('siscomextipodeclaracaolist.unpublish');
		}
		...
		//$default_order_field = 'codigo_tipo';
	...
	protected function auth($area){
		//echo '<pre>' . print_r(JAccess::getActions('com_siscomex','siscomextipodeclaracao'),true).'</pre>';die();
		$aclLocal = array();
		foreach(JAccess::getActions('com_siscomex', 'siscomextipodeclaracao') as $ar) $aclLocal[] = $ar->name;
		if(in_array($area, $aclLocal)){
			return $this->user->authorise($area, 'com_siscomex', 'siscomextipodeclaracao');
		}else{
			$aclGlobal = array();
