 ...
 * @subpackage 	Siscomex
 */
class SiscomexViewSiscomextipoembalagemlist extends JViewLegacy
{
	/**
	 * Siscomextipoembalagemlist view display method
	 * @return void
	 **/
	function display($tpl = null){
		...
		JToolBarHelper::title( JText::_( 'COM_SISCOMEX_MENU_SISCOMEXTIPOEMBALAGEMLIST' ), 'generic.png' );
		if($this->user->authorise('core.edit', 'com_siscomex')) JToolBarHelper::editList('siscomextipoembalagem.edit');
		if($this->user->authorise('core.create', 'com_siscomex')) JToolBarHelper::addNew('siscomextipoembalagem.add');
		if($this->user->authorise('core.delete', 'com_siscomex')) JToolBarHelper::deleteList('', 'siscomextipoembalagemlist.delete');
		
		if( (isset($this->rows[0]->published)) && ($this->user->authorise('core.edit', 'com_siscomex')) ){
			JToolBarHelper::divider();
			JToolBarHelper::publish('siscomextipoembalagemlist.publish');
			JToolBarHelper::unpublish('siscomextipoembalagemlist.unpublish');
		}
	...
	protected function auth($area){
		//echo '<pre>' . print_r(JAccess::getActions('com_siscomex','siscomextipoembalagem'),true).'</pre>';die();
		$aclLocal = array();
		foreach(JAccess::getActions('com_siscomex', 'siscomextipoembalagem') as $ar) $aclLocal[] = $ar->name;
		if(in_array($area, $aclLocal)){
			return $this->user->authorise($area, 'com_siscomex', 'siscomextipoembalagem');
		}else{
			$aclGlobal = array();
