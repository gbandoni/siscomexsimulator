 ...
 * @subpackage 	Siscomex
 */
class SiscomexViewSiscomexfundlegallist extends JViewLegacy
{
	/**
	 * Siscomexfundlegallist view display method
	 * @return void
	 **/
	function display($tpl = null){
		...
		JToolBarHelper::title( JText::_( 'COM_SISCOMEX_MENU_SISCOMEXFUNDLEGALLIST' ), 'generic.png' );
		if($this->user->authorise('core.edit', 'com_siscomex')) JToolBarHelper::editList('siscomexfundlegal.edit');
		if($this->user->authorise('core.create', 'com_siscomex')) JToolBarHelper::addNew('siscomexfundlegal.add');
		if($this->user->authorise('core.delete', 'com_siscomex')) JToolBarHelper::deleteList('', 'siscomexfundlegallist.delete');
		
		if( (isset($this->rows[0]->published)) && ($this->user->authorise('core.edit', 'com_siscomex')) ){
			JToolBarHelper::divider();
			JToolBarHelper::publish('siscomexfundlegallist.publish');
			JToolBarHelper::unpublish('siscomexfundlegallist.unpublish');
		}
	...
	protected function auth($area){
		//echo '<pre>' . print_r(JAccess::getActions('com_siscomex','siscomexfundlegal'),true).'</pre>';die();
		$aclLocal = array();
		foreach(JAccess::getActions('com_siscomex', 'siscomexfundlegal') as $ar) $aclLocal[] = $ar->name;
		if(in_array($area, $aclLocal)){
			return $this->user->authorise($area, 'com_siscomex', 'siscomexfundlegal');
		}else{
			$aclGlobal = array();
