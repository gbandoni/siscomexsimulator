 ...
 * @subpackage 	Siscomex
 */
class SiscomexViewSiscomexdocumentosdi extends JViewLegacy
{
	/**
	...
	function display($tpl = null){
		...
		JToolBarHelper::title(   JText::_( 'COM_SISCOMEX_MENU_SISCOMEXDOCUMENTOSDILIST' ).': <span>[ ' . $text.' ]</span>' );
		
		if ($isNew)  {
			if($user->authorise('core.create', 'com_siscomex')){
				JToolBarHelper::apply('siscomexdocumentosdi.apply');
				JToolBarHelper::save('siscomexdocumentosdi.save');
			}
			JToolBarHelper::cancel('siscomexdocumentosdi.cancel');
		} else {
			...
			JToolBarHelper::cancel( 'siscomexdocumentosdi.cancel', 'JTOOLBAR_CLOSE' );
		}
