 ...
 * @subpackage 	Siscomex
 */
class SiscomexViewSiscomexacrescimos extends JViewLegacy
{
	/**
	...
	function display($tpl = null){
		...
		JToolBarHelper::title(   JText::_( 'COM_SISCOMEX_MENU_SISCOMEXACRESCIMOSLIST' ).': <span>[ ' . $text.' ]</span>' );
		
		if ($isNew)  {
			if($user->authorise('core.create', 'com_siscomex')){
				JToolBarHelper::apply('siscomexacrescimos.apply');
				JToolBarHelper::save('siscomexacrescimos.save');
			}
			JToolBarHelper::cancel('siscomexacrescimos.cancel');
		} else {
			...
			JToolBarHelper::cancel( 'siscomexacrescimos.cancel', 'JTOOLBAR_CLOSE' );
		}
